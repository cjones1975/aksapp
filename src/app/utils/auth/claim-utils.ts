    ...
    switch (key) {
      ...
      case 'principal_name':
        populateClaim(
          key,
          claims[key],
          'The principal name that represents the user. It could be an email address, phone number, or a generic username without a specified format. Its value is mutable and might change over time. Since it is mutable, this value must not be used to make authorization decisions. It can be used for username hints, however, and in human-readable UI as a username. The profile scope is required in order to receive this claim.',
          claimsTable
        );
        break;
      case 'extension_CustomerCode':
        populateClaim(key, claims[key], 'The Sita customer code', claimsTable);
        break;
      case 'nonce':
        populateClaim(
          key,
